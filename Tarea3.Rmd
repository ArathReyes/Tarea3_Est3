---
title: "Tarea 3. Modelación ARIMA"
author: "Aguilar García Karen Shamed, Cruz Estrada Valeria Lucero, Reyes López Arath Alejandro"
date: "7/11/2021"
...
attributes(ts_12)
```
#### 1. *Análisis descriptivo*: Grafique los datos, describa lo que observe (varianza constante o no constante, descomposicíon clásica, tendencia, ciclos estacionales, periodicidad de los ciclos).
...
plot(cycle(ts_12))
```
De esta manera, es evidente que tenemos **ciclos estacionales de cada 12 meses o cada 4 registros.**
...
#### 2. *Missing data*: Suponga que las observaciones de 1971 Qtr1, 1973 Qtr2 y 1973 Qtr3, son datos faltantes NA, es decir, sustituya estas observaciones por NA. 

#### Use al menos dos métodos de imputacion de la paquetería imputeTS. ¿Cuál método es adecuado para estos datos? (Note que el valor imputado debe aproximarse al valor omitido).
...
#### 3. *Ajuste*: Con los datos observados completos, ajuste un modelo ARIMA o SARIMA adecuado.

#### Obtenga correlogramas, revise si los parámetros son significativos, compruebe los supuestos (que los residuales sean ruido blanco con distribuci ́on normal).

#### Obtenga dos o más posibles modelos, realice análisis de residuales y calcule medidas de bondad de ajuste. Haga la comparación para decidir cúalmodelo sería el más adecuado.
...
#Modelo 1
#SARIMA(1,2,1)(0,1,1)[4]
fit1_data <- arima(sqrt(ts_12), c(1, 2, 1), seasonal = list(order = c(0, 1, 1), period = 4)) # El que propuse
#Modelos 2
#SARIMA(0,0,1)(1,1,1)[4] 
fit2_data <- arima(data, c(0, 0, 1), seasonal = list(order = c(1, 1, 1), period = 4)) #antes era modelo 1
#Modelo 3
...
```
Es imperativo mencionar que el modelo 1, aquel que está dado por la serie original con raíz cuadrada, es tal que $d=2$ pues como observamos anteriormente el diferenciar dos veces nos proporciona una serie estacionaria, mientras que los demás modelos contienen a los datos diferenciados manualmente, esto lo hacemos así, pues si el modelo 1 resulta ser el mejor, nos facilitará regresar la transformación.


Ahora usemos la función *auto.arima()* para proponer un cuarto modelo:
...
```{r}
comparar_1=cbind("SARIMA(1,2,1)(0,1,1)[4]",fit1_data$aic,BIC(fit1_data), mean(fit1_data$residuals), mean(abs(fit1_data$residuals)), sqrt(mean((fit1_data$residuals)^2)), 4)
...
(resultados)
```
Considerando el AIC, notemos que el *modelo 5* se ajusta mejor y tomando en cuenta el BIC tenemos que el *modelo 4* se ajusta mejor. Pero considerando los errores tenemos que el *modelo 1* se ajusta mejor. Ahora verifiquemos los supuestos para elegir el mejor modelo.
...
Ahora, comparando el *modelo 1* y el *modelo 4*, podemos ver que el primero tiene mejores valores para *ME*, *MAE* y *RMSE* que el cuarto modelo. Por lo tanto nos quedamos con el **modelo 1**. Podemos notar que el modelo 1 pasa todas las pruebas de normalidad excepto por la de Jarque-Bera, por muy poco, y por ello consideramos a este el **mejor modelo**, y en cualquier caso creemos que pedir normalidad es un supuesto demasiado fuerte como para ignorar el desempeño de este modelo basado en el fallo de la prueba de Jarque-Bera.

#### 4. *Forecasting*: Con el modelo estimado, pronostique $n_{new} = 8$ (2 años) valores futuros (obtenga intervalos de predicción).
...
```{r}
### Predict
pred <- predict(fit1_data, n.ahead=8)$pred
pred
pred= pred^2
pred1 <- predict(fit1_data, n.ahead = 24)
tl <- pred1$pred - 1.96 * pred1$se
tu <- pred1$pred + 1.96 * pred1$se
tl <- (tl)^2
tu <- (tu)^2

plot(forecast(ts_12, 8))
ts.plot(ts_12, xlim= c(1964, 1979), ylim=c(9,50), main= "Predicción")
